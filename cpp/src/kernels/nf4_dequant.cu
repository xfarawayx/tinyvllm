...
torch::Tensor nf4_dequantize(
  ...
  auto output = torch::empty({num_rows, num_cols},
      torch::TensorOptions().device(packed_weights.device()).dtype(out_dtype));
  ...
  auto out_scalar = output.scalar_type();

  if (out_scalar == at::ScalarType::Half) {
  ...
  } else if (out_scalar == at::ScalarType::BFloat16) {
    ...
    TORCH_CHECK(false, "nf4_dequantize: unsupported output dtype (must be fp16 or bf16)");
  }
...
// ============================================================================
//  Host: NF4 linear (full dequantize + matmul)
// ============================================================================
